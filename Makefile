...
cli-test: ## Test CLI commands
	$(POETRY) run python -c "from flext_cli.cli import cli; print('CLI test passed')"
...
.DEFAULT_GOAL := help
.PHONY: help install install-dev setup validate check lint format type-check security fix test test-unit test-integration test-fast coverage-html cli-test build build-clean docs docs-serve deps-update deps-show deps-audit shell pre-commit clean clean-all reset diagnose doctor t l f tc c i v