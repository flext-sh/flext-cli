...
├── client-a                   # ❌ client-a project commands (Sprint 5)
├── client-b                # ❌ client-b project commands (Sprint 5)
...
Examples:
  flext config set auth.username REDACTED_LDAP_BIND_PASSWORD
...
#### **client-a Commands (`flext client-a`) - Sprint 5**

```bash
# client-a Oracle Unified Directory migration
flext client-a migration status [OPTIONS]
flext client-a oud sync [OPTIONS]
flext client-a pipeline deploy [OPTIONS] PIPELINE_NAME
flext client-a logs [OPTIONS]
```

#### **client-b Commands (`flext client-b`) - Sprint 5**

```bash
# client-b-specific Meltano operations
flext client-b pipeline deploy [OPTIONS] PIPELINE_NAME
flext client-b status [OPTIONS]
flext client-b logs [OPTIONS]
```
