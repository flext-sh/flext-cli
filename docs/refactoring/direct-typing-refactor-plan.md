...
- **Done**: Removed `cast_if`; `to_dict_json` / `to_list_json` use TypeAdapter.validate_python with ValidationError fallback only (no cast_if). `ensure_dict` uses `isinstance(result, dict)`; `get_map_val` uses `isinstance` for t.NormalizedValue compatibility.
...
- **Done**: settings.save_config accepts `FlextCliSettings | Mapping`, uses `to_save` from model_dump() or config; api.get_auth_token uses TokenData(data) as primary path, extract only on ValidationError; protocol save_config left as Mapping to avoid circular import (protocols → settings → utilities → models → protocols).
