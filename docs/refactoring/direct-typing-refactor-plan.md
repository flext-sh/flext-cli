...
- **Done**: Removed `cast_if`; `to_dict_json` / `to_list_json` use TypeAdapter.validate_python with ValidationError fallback only (no cast_if). `ensure_dict` uses `isinstance(result, dict)`; `get_map_val` uses `isinstance` for t.RecursiveContainer compatibility.
