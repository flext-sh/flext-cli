...
**6 findings** — high 0, medium 5, low 1 Confiança: high 4, medium 0, low 2
...
**Classe**: Improper Authentication · **Local**:
`src/flext_cli/_utilities/_xlxx/xlsx_protection.py:65`

> The password on 'protection' is being set without validating the password. Call
> django.contrib.auth.password_validation.validate_password() with validation functions
> before setting the password. See
> <https://docs.djangoproject.com/en/3.0/topics/auth/passwords/> for more information.
...
> namespaces. Make sure that no arbitrary value can be written as the annotation and
> passed to `typing.get_type_hints` function.
