# FLX CLI - ENTERPRISE COMMAND LINE INTERFACE

> **Universal CLI for FLX Meltano Enterprise platform operations** > **Status**: ⚠️ **Type Fixes Required** | **Health**: 🟡 **Functional** | **Updated**: 2025-06-23

## 🎯 OVERVIEW & PURPOSE

The FLX CLI module provides a **unified command-line interface** for the FLX Meltano Enterprise platform, delivering:

- **Universal Command Architecture**: Single entry point for all platform operations
- **gRPC Client Integration**: Direct communication with FLX daemon services
...
| **gRPC Communication**  | `FlxCliGrpcClient` class                                      | ⚠️ Type fixes needed |
...
| ---------------------------------- | ------------------------ | ------------------------------------- |
| **flx_core.universe**              | Universal command system | `universal_cli()`, `universal_grpc()` |
| **flx_core.grpc.client**           | gRPC base client         | `FlxGrpcClientBase` inheritance       |
| **flx_core.domain.advanced_types** | Enterprise types         | `ServiceResult`, `ServiceError`       |
...
def main() -> None:
    """Main entry point for flx-ent console script."""
    cli()
```
...
```mermaid
graph TD
    A[flx_cli] --> B[flx_core.universe]
    A --> C[flx_core.grpc.client]
    A --> D[flx_core.domain.advanced_types]
    A --> E[flx_core.config.domain_config]

    B --> F[universal_cli - Command routing]
    B --> G[universal_grpc - gRPC communication]
    C --> H[FlxGrpcClientBase - Base client]
...
| -------------------- | --------------- | ----------------- | ------------------------------------ |
| **FLX Daemon**       | gRPC            | Command execution | `config.network.grpc_host:grpc_port` |
| **Universal System** | Direct import   | Command routing   | `flx_core.universe`                  |
| **Configuration**    | Direct import   | Auto-discovery    | `flx_core.config.domain_config`      |

### 🔌 Usage Integration Points

```mermaid
graph LR
    A[Terminal] --> B[flx-ent command]
    B --> C[CLI Entry Point]
    C --> D[Universal CLI]
    D --> E[Command Router]
    E --> F[gRPC Client]
...
flx-ent create-pipeline --name "sales_etl" --extractor "tap-salesforce"
flx-ent list-pipelines
flx-ent execute-pipeline --pipeline-id "pipe-123"
flx-ent pipeline-status --pipeline-id "pipe-123"

# System operations
flx-ent health
flx-ent version
flx-ent config-validate

# Advanced operations
flx-ent pipeline-run --pipeline "sales_etl" --full-refresh
flx-ent plugin-install --name "tap-postgres" --type "extractor"
```

### 🔧 Programmatic Usage

```python
from flx_cli.client import FlxCliGrpcClient

# Initialize client with automatic configuration
client = FlxCliGrpcClient()
...
client = FlxCliGrpcClient("localhost:50051")
...
1. **Investigate universal_grpc API** - Determine correct function signature
   ...
   python -c "from flx_core.universe import universal_grpc; help(universal_grpc)"
   ```
...
| ------------------ | --------------------- | ------------------------------------------ |
| `create-pipeline`  | Create new pipeline   | `flx-ent create-pipeline --name sales_etl` |
| `list-pipelines`   | List all pipelines    | `flx-ent list-pipelines`                   |
| `execute-pipeline` | Run pipeline          | `flx-ent execute-pipeline --id pipe-123`   |
| `pipeline-status`  | Check pipeline status | `flx-ent pipeline-status --id pipe-123`    |
| `health`           | System health check   | `flx-ent health`                           |
| `version`          | Show version info     | `flx-ent version`                          |

### 🔧 Administrative Commands

| Command           | Purpose                | Example                                      |
| ----------------- | ---------------------- | -------------------------------------------- |
| `config-validate` | Validate configuration | `flx-ent config-validate`                    |
| `plugin-install`  | Install plugin         | `flx-ent plugin-install --name tap-postgres` |
| `plugin-list`     | List plugins           | `flx-ent plugin-list`                        |
| `daemon-status`   | Check daemon status    | `flx-ent daemon-status`                      |

### 🎯 Integration Commands

| Command            | Purpose                     | Example                                          |
| ------------------ | --------------------------- | ------------------------------------------------ |
| `export-config`    | Export configuration        | `flx-ent export-config --format yaml`            |
| `import-pipelines` | Import pipeline definitions | `flx-ent import-pipelines --file pipelines.yaml` |
| `backup-create`    | Create system backup        | `flx-ent backup-create --output backup.tar.gz`   |
| `logs-tail`        | Tail system logs            | `flx-ent logs-tail --follow`                     |

---

**🎯 SUMMARY**: The FLX CLI provides a powerful, type-safe command-line interface with enterprise-grade error handling and universal command architecture. The 6 type errors require immediate resolution, but the core functionality and architecture are solid and production-ready.
