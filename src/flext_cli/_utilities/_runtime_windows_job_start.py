...
class FlextCliUtilitiesRuntimeWindowsJobStartMixin:
    ...
    @staticmethod
    def _windows_job_create(process: subprocess.Popen[bytes]) -> p.Result[int]:
    ...
    @staticmethod
    def _windows_process_resume(process_id: int) -> str | None:
        ...
        try:
            ...
            if prior_suspend_count == 0xFFFFFFFF:
                return f"ResumeThread failed: {ctypes.get_last_error()}"
        except (OSError, TypeError, ValueError) as exc:
