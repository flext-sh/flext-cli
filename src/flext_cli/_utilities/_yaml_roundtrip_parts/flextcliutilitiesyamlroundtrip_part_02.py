"""Round-trip YAML tree-editing helpers behind ``u.Cli.yaml_*`` (part 02).
...
order-preserving overlays. Lifted from the cosmos-charts ``yaml_utils``
contract into the shared facade.

NOTE (multi-agent): composed into ``FlextCliUtilitiesYaml`` via MRO in
``yaml.py``; consumes part 01 through the ``FlextCliUtilitiesYamlRoundtripPart01``
alias (same pattern as the toml parts). Do not duplicate these operations in a
leaf module.
...
class FlextCliUtilitiesYamlRoundtrip:
    ...
    def yaml_clear_anchors(node: t.Cli.YamlValue) -> None:
        ...
        if node is None:
            return
        if FlextCliUtilitiesYamlRoundtrip._yaml_has_anchor(node):
            node.yaml_set_anchor(None)
        if isinstance(node, Mapping):
            for value in node.values():
                FlextCliUtilitiesYamlRoundtrip.yaml_clear_anchors(value)
        elif FlextCliUtilitiesYamlRoundtripPart01.yaml_is_sequence(node):
            for item in node:
                FlextCliUtilitiesYamlRoundtrip.yaml_clear_anchors(item)
    ...
    def yaml_has_key_comment(node: CommentedMap, key: str, text: str) -> bool:
        ...
        """
        wanted = FlextCliUtilitiesYamlRoundtrip._yaml_comment_core(text)
        return any(
            FlextCliUtilitiesYamlRoundtrip._yaml_comment_core(token.value) == wanted
            for token in FlextCliUtilitiesYamlRoundtrip.yaml_pre_key_tokens(node, key)
        )
    ...
    def yaml_add_pre_key_comment(
        ...
        if FlextCliUtilitiesYamlRoundtrip.yaml_has_key_comment(node, key, text):
            return
        comment_text = FlextCliUtilitiesYamlRoundtrip._yaml_comment_core(text)
    ...
    def yaml_force_block_style(node: t.Cli.YamlNode) -> None:
        ...
        if isinstance(node, CommentedMap):
            ...
            for value in node.values():
                FlextCliUtilitiesYamlRoundtrip.yaml_force_block_style(value)
        elif isinstance(node, CommentedSeq):
            if not node:
                return
            node.fa.set_block_style()
            for item in node:
                FlextCliUtilitiesYamlRoundtrip.yaml_force_block_style(item)
    ...
    def yaml_overlay_preserving_order(
        ...
        for key, value in overlay.items():
            if key in base:
                ...
                new_keys.append((
                    key,
                    FlextCliUtilitiesYamlRoundtripPart01.yaml_deep_to_commented(value),
                ))
