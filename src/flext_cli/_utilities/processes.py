...
class FlextCliUtilitiesProcesses:
    ...
    def process_start(
    ...
    ) -> p.Result[FlextCliManagedProcess]:
        ...
        try:
            ...
            return r[FlextCliManagedProcess].fail(
                f"execution error: {shlex.join(list(cmd))}: {exc}",
            )
        return r[FlextCliManagedProcess].ok(
            FlextCliManagedProcess(process, cwd=cwd, env=resolved_env),
        )


__all__: list[str] = ["FlextCliManagedProcess", "FlextCliUtilitiesProcesses"]
