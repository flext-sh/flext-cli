...
class TestAuthUtilities:
    ...
    def test_refresh_auth_token_success(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(mode="w", delete=False) as refresh_file:
            ...
            with tempfile.NamedTemporaryFile(delete=False) as token_file:
                ...
                with patch("flext_cli.cli_auth.create_token_provider") as mock_create:
                    ...
                    mock_provider.refresh_token.return_value = FlextResult[str].ok("new_token")
                    ...
                    if result.success:
                        assert "new_token" in result.unwrap() or "refresh" in result.unwrap()
                    else:
...
class TestAuthIntegration:
    ...
    def test_full_auth_workflow_success(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            refresh_result = save_refresh_token("refresh_token", token_path=refresh_path)
