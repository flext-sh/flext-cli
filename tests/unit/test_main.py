...
class TestFlextCliMain:
    ...
    def test_nested_groups(self, cli_main: FlextCliMain, runner: CliRunner) -> None:
        ...
        @cli_main.group()
        def REDACTED_LDAP_BIND_PASSWORD() -> None:
            """Admin commands."""

        @REDACTED_LDAP_BIND_PASSWORD.group()
        def users() -> None:
            """User management."""
        ...
        result = runner.invoke(
            cast("click.Command", main_group.unwrap()), ["REDACTED_LDAP_BIND_PASSWORD", "users", "list-users"]
        )
        assert result.exit_code == 0
