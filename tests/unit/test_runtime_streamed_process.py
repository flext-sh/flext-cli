"""Public core contracts for streamed process execution."""
...
class TestsFlextCliRuntimeStreamedProcess:
    """Prove the public model, compatibility, bytes, and sink contracts."""
    ...
    def test_combined_output_is_byte_exact_and_live(
        ...
        tm.that(
            any(
                thread.name.startswith("flext-cli-")
                for thread in threading.enumerate()
            ),
            eq=False,
        )

    def test_non_utf8_combined_output_is_byte_exact(
        self,
        tmp_path: Path,
        capfdbinary: pytest.CaptureFixture[bytes],
    ) -> None:
        """Preserve arbitrary child bytes without decoding either sink."""
        output_file = tmp_path / "non-utf8.log"
        stdout_bytes = b"\xff\xfe\x00stdout\n"
        stderr_bytes = b"\x80\x81\x00stderr\n"
        script = (
            "import os;"
        ...
        expected = stdout_bytes + stderr_bytes
        tm.ok(result)
        tm.that(result.value, eq=0)
        tm.that(output_file.read_bytes(), eq=expected)
        tm.that(capfdbinary.readouterr().out, eq=expected)
    ...
    def test_deadline_model_satisfies_public_protocol(self) -> None:
        ...
        tm.that(_deadline(seconds=2.0, grace=0.5), is_=p.Cli.ProcessDeadline)

    def test_legacy_timeout_contract_remains_a_failure(self, tmp_path: Path) -> None:
        """Keep the existing relative-timeout failure contract."""
        result = u.Cli().run_to_file(
