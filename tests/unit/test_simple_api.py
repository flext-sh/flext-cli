...
class TestCreateDevelopmentCLIConfig:
    ...
    def test_create_development_config_partial_overrides(self) -> None:
        ...
        overrides = {"timeout": 60, "project_version": "0.8.0"}
...
class TestCreateProductionCLIConfig:
    ...
    def test_create_production_config_with_overrides(self) -> None:
        ...
        overrides = {
            "api_url": "https://custom.flext.com",
            "debug": True,
            "timeout": 120,
        }
        ...
        # Check overridden values
        assert config.api_url == "https://custom.flext.com"
        assert config.debug is True
    ...
    def test_create_production_config_all_overrides(self) -> None:
        """Test creating production config with all values overridden."""
        overrides = {
            "debug": True,
        ...
        assert config.debug is True
        assert config.project_name == "custom-cli"
...
class TestGetCLISettings:
    ...
    def test_get_cli_settings_is_consistent(self) -> None:
        ...
        # Should have the same values (not necessarily the same instance)
...
class TestConfigValidation:
    ...
    def test_config_invalid_values_handled(self) -> None:
        """Test that invalid configuration values are handled properly."""
        # This should work fine - Pydantic will handle type conversion
        config = create_development_cli_config(
            timeout="30",  # String instead of int
            debug="true",  # String instead of bool
        )

        # Pydantic should convert these appropriately
...
class TestConfigurationTypes:
    ...
    def test_development_config_types(self) -> None:
        """Test development config has correct types."""
        config = create_development_cli_config()
    ...
    def test_production_config_types(self) -> None:
        """Test production config has correct types."""
        config = create_production_cli_config()
