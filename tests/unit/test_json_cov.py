...
class TestsFlextCliJsonCov:
    ...
    def test_json_read_missing_file_fails_loudly(self, tmp_path: Path) -> None:
        """Reject a missing JSON file with its path in the public error."""
        missing = tmp_path / "missing.json"
        result = u.Cli.json_read(missing)
        tm.fail(result)
    ...
    def test_json_read_valid_object_returns_parsed_mapping(
        ...
        """Return a parsed mapping for a valid JSON object file."""
    ...
    @pytest.mark.parametrize(
        ...
        [("not json!!", "json_read"), ("[1, 2, 3]", "root must be an object")],
    )
    def test_json_read_rejects_invalid_content(
        self, tmp_path: Path, content: str, expected_error: str
    ) -> None:
        """Reject malformed JSON and non-object roots with context."""
    ...
    def test_json_write_then_read_roundtrips_payload(self, tmp_path: Path) -> None:
        """Round-trip a mapping through the public file helpers."""
    ...
    def test_json_write_sort_keys_orders_nested_keys(self, tmp_path: Path) -> None:
        """Sort keys recursively when the write option is enabled."""
    ...
    def test_json_write_serializes_pydantic_model_as_object(
        ...
        """Serialize a Pydantic model as a JSON object."""
    ...
    def test_json_as_mapping_coerces_to_mapping_or_empty(
        ...
        """Coerce mapping values and return empty for other shapes."""
    ...
    def test_json_as_sequence_coerces_to_list_or_empty(
        ...
        """Coerce sequence values and return empty for other shapes."""
    ...
    def test_json_as_mapping_list_filters_to_mappings(
        ...
        """Retain only mappings from a JSON sequence."""
    ...
    def test_json_walk_path_returns_leaf_for_existing_path(self) -> None:
        """Return the leaf value for an existing mapping path."""
    ...
    def test_json_walk_path_returns_none_when_unreachable(
        ...
        """Return none when a mapping path cannot be traversed."""
    ...
    def test_json_deep_mapping_descends_into_nested_object(self) -> None:
        """Return a nested mapping reached by successive keys."""
    ...
    def test_json_pick_str_trims_and_falls_back(self) -> None:
        """Trim string values and use the explicit default when absent."""
    ...
    def test_json_pick_int_coerces_scalar_variants(
        ...
        """Coerce supported scalar variants to integers."""
        data = u.Cli.json_as_mapping(
            u.Cli.json_loads(
    ...
    def test_json_pick_bool_coerces_truthy_variants(
        ...
        """Coerce supported scalar variants to booleans."""
        data = u.Cli.json_as_mapping(
            u.Cli.json_loads(
        ...
        tm.that(u.Cli.json_pick_bool(data, key), eq=expected)

    def test_json_pick_bool_uses_default_for_missing_key(self) -> None:
        """Use the explicit boolean default for a missing value."""
    ...
    def test_json_nested_int_reads_nested_value_or_default(self) -> None:
        """Read a nested integer or use the explicit default."""
    ...
    def test_json_get_str_key_trims_value(self) -> None:
        """Return a trimmed string from a mapping key."""
