...
class TestsFlextCliFileIteration:
    ...
    def test_git_scope_preserves_visible_state_and_path_semantics(
        ...
        selected = u.Cli.iter_matching_files(
            repository,
            includes=("*.py",),
        ...
        nested_selected = u.Cli.iter_matching_files(
            nested, includes=("*.py",), excludes=()
        )
        tm.that(nested_selected, eq=sorted((dirty, excluded, untracked)))

    def test_non_git_scope_falls_back_to_recursive_globs(
        self, tmp_path: Path
    ) -> None:
        ...
        for path in (top_level, nested_match, nested_excluded, unrelated):
            path.write_text(path.name, encoding="utf-8")
