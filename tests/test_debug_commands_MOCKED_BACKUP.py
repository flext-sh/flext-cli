...
class TestDebugCommands:
    ...
    def setup_method(self) -> None:
        ...
        self.console = Console(file=None, width=80)  # Real console for testing

    def test_connectivity_command(self) -> None:
        """Test connectivity command with real functionality."""
        # Test real connectivity command execution
        result = self.runner.invoke(debug_cmd, ["connectivity"], obj={"console": self.console})
        
        # Command should complete (either success or controlled failure)
    ...
    def test_performance_command(self) -> None:
        ...
        # Test real performance command execution
        result = self.runner.invoke(debug_cmd, ["performance"], obj={"console": self.console})
        
        # Command should complete (either success or controlled failure)
        assert result.exit_code in [0, 1]  # Success or expected API failure

    def test_validate_command_success(self) -> None:
        """Test validate command with success."""
        with (
            patch("flext_cli.cmd_debug.get_config") as mock_get_config,
            patch("flext_cli.cmd_debug.sys.version_info", (3, 11, 0)),
            patch("flext_cli.cmd_debug.sys.version", "3.11.0 (main, Oct 24 2022)"),
        ):
    ...
    def _test_validate_command_success_impl(self, mock_get_config: MagicMock) -> None:
        ...
        # Mock config
        mock_config = self.console
        mock_config.config_dir = Path("/home/user/.flext")
        mock_get_config.return_value = mock_config

        # Test real validate command execution
        result = self.runner.invoke(
            debug_cmd,
            ["validate"],
            obj={"console": self.console},
        )

        # Should complete (either success or controlled failure)
    ...
    def test_validate_command_old_python(self) -> None:
        ...
        with (
            patch("flext_cli.cmd_debug.get_config") as mock_get_config,
            patch("flext_cli.cmd_debug.sys.version_info", (3, 9, 0)),
            patch("flext_cli.cmd_debug.sys.version", "3.9.0 (main, Oct 24 2021)"),
        ):
    ...
    def _test_validate_command_old_python_impl(
        ...
        # Test real validate command with mocked Python version
        result = self.runner.invoke(
            debug_cmd,
            ["validate"],
            obj={"console": self.console},
        ...
        # Should complete (may pass or fail based on real environment)
        assert result.exit_code in [0, 1]  # Success or validation error
    ...
    def test_paths_command(self) -> None:
        ...
        with (
            patch("flext_cli.cmd_debug.get_config") as mock_get_config,
            patch("flext_cli.cmd_debug.Path") as mock_path_class,
        ):
...
class TestDebugFunctionality:
    ...
    def test_config_access(self) -> None:
        ...
        with patch("flext_cli.cmd_debug.get_config") as mock_get_config:
    ...
    def test_click_context_pattern(self) -> None:
        ...
        # Test that we can create a real context like used in commands
...
class TestDebugCommandErrorHandling:
    ...
    def test_connectivity_with_exception(self) -> None:
        """Test connectivity command with real exception handling."""
        # Test real connectivity command with potential network failures
        result = self.runner.invoke(debug_cmd, ["connectivity"], obj={"console": self.console})
    ...
    def test_performance_with_exception(self) -> None:
        ...
        # Test real performance command with potential API failures
        result = self.runner.invoke(debug_cmd, ["performance"], obj={"console": self.console})
