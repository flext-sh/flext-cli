...
class TestCliUtilsFileOperations:
    ...
    def test_cli_load_data_file_csv_real(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert loaded_data[2]["department"] == "Marketing", "Third row department should be Marketing"
...
class TestCliUtilsBatchProcessing:
    ...
    def test_cli_batch_process_files_real(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(successful_results) == 3, "Should process all 3 files successfully"
            
            # Verify file paths are in the successful list
            for i, file_path in enumerate(path_objects):
                assert str(file_path) in successful_results, f"File {file_path} should be in successful results"
    ...
    def test_cli_quick_setup_real(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                assert "test-project" in content, "pyproject.toml should contain project name"
            finally:
...
class TestCliUtilsErrorHandling:
    ...
    def test_load_nonexistent_file_real(self) -> None:
        ...
        assert "not" in result.error.lower() or "exist" in result.error.lower(), "Error should mention file doesn't exist"
...
class TestCliUtilsDataTypes:
    ...
    def test_complex_data_structures_real(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert loaded_data == complex_data, "Complex data should round-trip correctly"
            ...
            assert loaded_yaml_data == complex_data, "Complex YAML data should round-trip correctly"
