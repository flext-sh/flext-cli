...
class TestFlextCliHelper:
    ...
    def test_flext_cli_validate_path_existing_file(self) -> None:
        ...
        with tempfile.NamedTemporaryFile() as temp_file:
            result = helper.flext_cli_validate_path(temp_file.name, must_be_file=True, must_exist=True)
    ...
    def test_flext_cli_validate_path_nonexistent(self) -> None:
        ...
        result = helper.flext_cli_validate_path("/nonexistent/file.txt", must_be_file=True, must_exist=True)
    ...
    def test_flext_cli_validate_path_directory(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = helper.flext_cli_validate_path(temp_dir, must_be_dir=True, must_exist=True)
    ...
    def test_flext_cli_validate_path_directory_as_file_error(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = helper.flext_cli_validate_path(temp_dir, must_be_file=True, must_exist=True)
...
class TestFlextCliDataProcessor:
    ...
    def test_processor_workflow_simple_case(self) -> None:
        ...
        def double_values_step(item: dict[str, object]) -> FlextResult[dict[str, object]]:
    ...
    def test_processor_validate_and_transform_valid_data(self) -> None:
        ...
        validators = {"name": "none", "value": "none"}  # Use 'none' for no-op validation
        transformers = {
            "name": lambda x: str(x).upper() if isinstance(x, str) else str(x),
            "value": lambda x: x * 2 if isinstance(x, (int, float)) else x
        }
        
        result = processor.flext_cli_validate_and_transform(data, validators, transformers)
    ...
    def test_processor_transform_data_pipeline_success(self) -> None:
        ...
        def filter_stage(d: dict[str, object]) -> FlextResult[dict[str, object]]:
            if "numbers" in d and isinstance(d["numbers"], list):
                filtered = [x for x in d["numbers"] if isinstance(x, (int, float)) and x > 5]
...
def test_flext_cli_batch_validate_simple_case():
    ...
    inputs = {"field1": ("test1", "none"), "field2": ("test2", "none"), "field3": ("test3", "none")}
