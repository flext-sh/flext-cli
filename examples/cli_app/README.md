...
python cli.py projects client-a status
```
...
python cli.py auth login --username REDACTED_LDAP_BIND_PASSWORD --password secret
...
```bash
# client-a project commands
python cli.py projects client-a migration start --type oud
python cli.py projects client-a status --detailed

# client-b project commands
python cli.py projects client-b sync --environment staging
python cli.py projects client-b health --all-services
...
- **Projects**: client-a, client-b, Meltano integration
