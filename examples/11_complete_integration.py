...
class DataManagerCLI:
    ...
    def add_entry(self) -> r[dict[str, t.NormalizedValue]]:
        ...
        converted_entry: dict[str, t.NormalizedValue] = {key: value}
        return r[dict[str, t.NormalizedValue]].ok(converted_entry)

    def display_data(self, data: dict[str, t.NormalizedValue]) -> None:
    ...
    def load_data(self) -> r[dict[str, t.NormalizedValue]]:
        """Load data with error handling. Uses read_json_dict for dict-only result."""
        if not self.data_file.exists():
            return r[dict[str, t.NormalizedValue]].fail("No data file found")
        ...
        if read_result.is_failure:
            ...
            return r[dict[str, t.NormalizedValue]].fail(error_msg)
        self.cli.print("✅ Data loaded successfully", style="green")
        return r[dict[str, t.NormalizedValue]].ok(read_result.value)
    ...
    def run_workflow(self) -> r[bool]:
        ...
        current_data: dict[str, t.NormalizedValue] = {}
    ...
    def save_data(self, data: dict[str, t.NormalizedValue]) -> r[bool]:
...
def process_with_railway_pattern(
    ...
    step1_data: dict[str, t.NormalizedValue] = {**input_data, "validated": True}
    step2_data: dict[str, t.NormalizedValue] = {**step1_data, "processed": True}
    final_data: dict[str, t.NormalizedValue] = {**step2_data, "enriched": True}
    result: r[dict[str, t.NormalizedValue]] = r[dict[str, t.NormalizedValue]].ok(
        final_data
    )
...
def main() -> None:
    ...
    test_data: dict[str, t.NormalizedValue] = {"id": 1, "name": "test"}
